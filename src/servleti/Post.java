	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("add".equals(action)){
			...
			if(addReply(request, user)) {
				data.put("message", "Uspesno postavljen odgovor!");
				data.put("status", "success");	
			}else{
				data.put("message", "Doslo je do greske!");
			...
			if(editReply(request, user, users)){
				data.put("message", "Odgovor uspesno izmenjen!");
				data.put("status", "success");	
			}else{
				data.put("message", "Doslo je do greske!");
				data.put("status", "fail");
			...
			if(delReply(request, user, users)){
				data.put("message", "Odgovor uspesno obrisan!");
				data.put("status", "success");	
			}else{
				data.put("message", "Doslo je do greske!");
				data.put("status", "fail");
	...
	private boolean addReply(HttpServletRequest request, User user){
		...
		try {
			...
			if(content.equals("") || content.length() <= 15){
				throw new Exception("Objava mora da sadrzi bar 15 karatkera!");
			}
