...
<%
	...
	if(forum.getType().getId() != 1 && user == null){
		request.setAttribute("message", "Niste ovlasteni da vidite ovo!");
		...
		<div class="container" style="margin-top: 10px">
			...
			<div class="page-header page-heading">
				...
				<div class="pull-right" id="logged_out" <%= logged_out %> style="width:100px;">
					...
					<button class="btn-default btn-sm btn-block" id="btn_login">Prijavite se</button>
					<button class="btn-default btn-sm btn-block" id="btn_register">Registrovanje</button>
				...
				<div class="pull-right" id="logged_in" <%= logged_in %>>
					...
					<button class="btn-default btn-sm" id="btn_logout">Izlogujte se</button>
				</div>

				<!-- Search box -->
				<div style="width: 200px;">
					<form action="./Search" method="post">
			            <input style="float:left; width:79%;" type="text" class="form-control" placeholder="Pretraga" name="keyword" id="srch-term">
				...
				<thead>
					<tr>
						...
						<th>
							...
							%>
								<h3 style="display: inline;">Nema podforuma</h3> &nbsp;
								<% if(!forum.isLocked()){ %>
							...
							%>
								<h3 style="display: inline;">PodForumi</h3> &nbsp;
								<% if(!forum.isLocked()){ %>
									<% if(user != null && user.getRole().getId() == 1){ %>
										<span class="glyphicon glyphicon-plus white add_forum" style="cursor: pointer; display: <%= display %>;"><span>
									<% } %>
								...
								<th class="cell-stat text-center hidden-xs hidden-sm white">Teme</th>
								<th class="cell-stat text-center hidden-xs hidden-sm white">Datum Kreiranja</th>
								<th class="cell-stat-2x hidden-xs hidden-sm white">Vlasnik</th>
								<% if(user != null && user.getRole().getId() == 1){ %>
									<th class="cell-stat-2x white" >Izmene</th>
								<% } %>
					...
					<tr>
						...
						<th>
							<%
								if (topics.isEmpty()){
							%>
							<h3 style="display: inline;">Nema Tema</h3> &nbsp;
							...
							%>
							<h3 style="display: inline;">Teme</h3> &nbsp;
								<% if(!forum.isLocked() && user != null && !user.isBanned()){ %>
									<span class="glyphicon glyphicon-plus white add_topic" style="cursor: pointer; display: <%= display %>;"><span>
								<% } %>
							</th>
							...
							<th class="cell-stat text-center hidden-xs hidden-sm white">Odgovori</th>
							<th class="cell-stat text-center hidden-xs hidden-sm white">Datum Kreiranja</th>
							<th class="cell-stat-2x hidden-xs hidden-sm white">Vlasnik</th>
				...
				<div style="width: 80%; margin: auto; padding:30px">
					 ...
					 <form>
						  ...
						  <div class="form-group form-group-sm">
  						  ...
  						  <div class="form-group form-group-sm">
						    <label for="pwd">Potvrdi lozinku:</label>
						    <span class="glyphicon glyphicon-asterisk" style="float: right;font-size:10px;"></span>
						  ...
						  <button type="submit" class="btn btn-default" id="register_btn">Registruj se</button>
						  <button type="submit" class="btn btn-default btn_black_win_container_close">Zatvori</button>
					</form>
				</div>
			</div>
			...
			<div class="addForum_container">
				<div style="width: 80%; margin: auto; padding:30px">
						 <form>
							  ...
							  <div class="form-group form-group-sm">
							    <label style="display: block;">Opis:</label>
							  ...
							  <button type="submit" class="btn btn-default" id="addForum_btn_add">Dodaj</button>
							  <button type="submit" class="btn btn-default btn_black_win_container_close">Zatvori</button>
						</form>
					</div>
			</div>
			...
			<div class="editForum_container">
				<div style="width: 80%; margin: auto; padding:30px">
						 <form>
							  ...
							  <button type="submit" class="btn btn-default" id="editForum_btn_edit">Izmeni</button>
							  <button type="submit" class="btn btn-default btn_black_win_container_close">Zatvori</button>
						</form>
					</div>
			</div>
...
<!-- ADVANCED SEARCH WINDOW -->
			<div class="advancedSearch_container">
				<div style="width: 80%; margin: auto; padding:30px">
						 <form method="post" action="Search">
							<div class="form-group form-group-sm">
								<label for="keyword">Kljucna rec:</label>
							...
							<div class="checkbox">
								...
								<label><input type="checkbox" id="titleCB" name="title">Tema</label> &nbsp;
								<label><input type="checkbox" id="contentCB" name="content">Odgovori</label> &nbsp;
								<label><input type="checkbox" id="ownerCB" name="user">Korisnik</label> &nbsp;
								<label><input type="checkbox" id="dateCB" name="date">Datum</label> </br></br>
							...
							<button type="submit" class="btn btn-default" id="searchBtn" onclick="getDateValues()">Pretrazi</button>
							<button type="submit" class="btn btn-default btn_black_win_container_close">Zatvori</button>
						</form>
					</div>
			</div>
		...
		<script src="js/moj.js" type="text/javascript"></script>
